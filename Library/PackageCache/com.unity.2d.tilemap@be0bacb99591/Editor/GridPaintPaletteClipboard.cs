...
namespace UnityEditor.Tilemaps
{
    ...
    internal class GridPaintPaletteClipboard : PaintableGrid
    {
        ...
        public Rect guiRect
        {
            ...
            set
            {
                ...
                if (float.IsNaN(value.x)
                    ...
                    || float.IsNaN(value.size.x)
                    || float.IsInfinity(value.size.x))
                    return;
        ...
        private static readonly PrefColor k_GridColor = new PrefColor("2D/Tile Palette Grid"
            , 255.0f / 255.0f
            , 255.0f / 255.0f
            , 255.0f / 255.0f
            , 76.5f / 255.0f
        ...
        public enum ClipboardParentSource
        {
            None = 0,
            Window = 1,
            Overlay = 2,
        };
        private ClipboardParentSource m_ClipboardParentSource = ClipboardParentSource.None;

        public ClipboardParentSource clipboardParentSource
        {
            get => m_ClipboardParentSource;
            set => m_ClipboardParentSource = value;
        }
        ...
        public void ClipboardOnInit()
        {
            SetupPreviewCameraOnInit();
            if (palette != null)
                unlocked = LoadUnlockState();
        }

        private bool LoadUnlockState()
        {
            return SessionState.GetBool($"PaletteClipboard_Unlocked_{m_ClipboardParentSource}", false);
        }

        private void SaveUnlockState(bool unlockState)
        {
            SessionState.SetBool($"PaletteClipboard_Unlocked_{m_ClipboardParentSource}", unlockState);
        }

        internal static void ClearUnlockedState()
        {
            foreach (var e in Enum.GetValues(typeof(ClipboardParentSource)))
            {
                SessionState.EraseBool($"PaletteClipboard_Unlocked_{e}");
            }
        }

        private void SetupPreviewCameraOnInit()
        {
            var saved = SessionState.GetBool($"PaletteClipboard_Saved_{m_ClipboardParentSource}", false);
            if (saved)
        ...
        {
            var position = SessionState.GetVector3($"PaletteClipboard_Position_{m_ClipboardParentSource}", new Vector3(0f, 0f, -10f));
            var swizzle = SessionState.GetInt($"PaletteClipboard_Swizzle_{m_ClipboardParentSource}", (int) Grid.CellSwizzle.XYZ);
            var orthographicSize = SessionState.GetFloat($"PaletteClipboard_Size_{m_ClipboardParentSource}", 5f);

            if (float.IsNaN(orthographicSize) ||
                float.IsInfinity(orthographicSize))
                orthographicSize = 5f;

            var xyz = Grid.InverseSwizzle((GridLayout.CellSwizzle)swizzle, position);
            m_PreviewUtility.camera.transform.position = GetCameraPositionFromXYZ(xyz);
            m_PreviewUtility.camera.orthographicSize = orthographicSize;
            m_PreviewUtility.camera.nearClipPlane = 0.01f;
            m_PreviewUtility.camera.farClipPlane = 100f;
            m_PreviewUtilityGizmo.camera.transform.position = position;
            m_PreviewUtilityGizmo.camera.orthographicSize = orthographicSize;
        ...
        {
            if (float.IsNaN(m_PreviewUtility.camera.orthographicSize) ||
                float.IsInfinity(m_PreviewUtility.camera.orthographicSize) ||
                m_ClipboardParentSource == ClipboardParentSource.None)
                return;

            SessionState.SetVector3($"PaletteClipboard_Position_{m_ClipboardParentSource}", m_PreviewUtility.camera.transform.position);
            SessionState.SetInt($"PaletteClipboard_Swizzle_{m_ClipboardParentSource}", (int) m_CameraSwizzleView);
            SessionState.SetFloat($"PaletteClipboard_Size_{m_ClipboardParentSource}", m_PreviewUtility.camera.orthographicSize);
            SessionState.SetBool($"PaletteClipboard_Saved_{m_ClipboardParentSource}", true);
        }

        internal static void ClearSavedCameraPosition()
        {
            foreach (var e in Enum.GetValues(typeof(ClipboardParentSource)))
            {
                SessionState.EraseVector3($"PaletteClipboard_Position_{e}");
                SessionState.EraseInt($"PaletteClipboard_Swizzle_{e}");
                SessionState.EraseFloat($"PaletteClipboard_Size_{e}");
                SessionState.EraseBool($"PaletteClipboard_Saved_{e}");
            }
        }
        ...
        {
            SaveUnlockState(unlocked);
            if (unlocked && tilemap != null)
                tilemap.ClearAllEditorPreviewTiles();
            SavePaletteIfNecessary();
            DestroyPreviewInstance();
        ...
        {
            if (Mathf.Approximately(oldSize.height, newSize.height)
                && Mathf.Approximately(oldSize.width, newSize.width))
                return;

            if (Mathf.Approximately(newSize.width * newSize.height, 0f))
                return;
        ...
        public void HandlePointerUpEvent(PointerUpEvent evt)
        {
            ...
            // Brush Move
            if (IsMoveEvent(evt) && m_PreviousMove.HasValue && isHotControl)
            {
                m_PreviousMove = null;
            ...
            // Brush Box
            if (evt.button == 0 && TilemapEditorTool.IsActive(typeof(BoxTool)))
            {
                if (isHotControl && m_MarqueeStart.HasValue && IsMouseUpInWindow())
                {
                    RegisterUndo();
                    var rect = GridEditorUtility.GetMarqueeRect(m_MarqueeStart.Value, mouseGridPosition);
                    if (evt.shiftKey)
        ...
        public void HandlePointerEnterEvent(PointerEnterEvent evt)
        {
            if (m_PreviousMousePosition.HasValue && !guiRect.Contains(m_PreviousMousePosition.Value) || !m_PreviousMousePosition.HasValue)
            {
                ...
                // PointerEnterEvent does not capture buttons for this event
                var button = evt.button;
                var alt = evt.altKey;
                var imguiEvt = Event.current;
                if (imguiEvt != null)
                {
                    button = imguiEvt.button;
                    alt = imguiEvt.alt;
                }
                if (!MousePanningEvent(button, alt) && GUIUtility.hotControl == m_MousePanningID)
                {
                    ClampZoomAndPan();
                    GUIUtility.hotControl = 0;
                }
        ...
        public void HandleWheelEvent(Vector3 delta, Vector2 currentMousePosition, bool shift)
        {
            ...
            Vector3 localDeltaXYZ = newLocalPos - oldLocalPos;
            var localDelta = Grid.Swizzle(m_CameraSwizzleView, localDeltaXYZ);
